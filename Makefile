...
FOLDER_BUILD=original_sw/build


# Host compiler global settings
CXXFLAGS += -fmessage-length=0
LDFLAGS += -lrt -lstdc++ -L$(FOLDER_BUILD) -lwfa
...
LDCLFLAGS += --config wfa.ini
EXECUTABLE = host
CMD_ARGS = $(BUILD_DIR)/wfa.xclbin
EMCONFIG_DIR = $(TEMP_DIR)
EMU_DIR = $(SDCARD)/data/emulation

BINARY_CONTAINERS += $(BUILD_DIR)/wfa.xclbin
BINARY_CONTAINER_wfa_OBJS += $(TEMP_DIR)/wfa.xo

# Setting higher frequency to achieve higher bandwidth
CLFLAGS+=--kernel_frequency 225
...
# Building kernel
$(TEMP_DIR)/wfa.xo: src/wfa_v2.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k wfa -I'$(<D)' -o'$@' '$<'
$(BUILD_DIR)/wfa.xclbin: $(BINARY_CONTAINER_wfa_OBJS)
	mkdir -p $(BUILD_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(BUILD_DIR) -l $(LDCLFLAGS) -o'$@' $(+)

# Building Host
$(EXECUTABLE): check-xrt $(HOST_SRCS) $(HOST_HDRS)
	$(CXX) $(CXXFLAGS) $(HOST_SRCS) $(HOST_HDRS) -Ioriginal_sw/ -o '$@' $(LDFLAGS)
...
ifeq ($(HOST_ARCH), x86)
	...
	XCL_EMULATION_MODE=$(TARGET) ./$(EXECUTABLE) $(BUILD_DIR)/wfa.xclbin
else
	mkdir -p $(EMU_DIR)
...
ifeq ($(HOST_ARCH), x86)
	./$(EXECUTABLE) $(BUILD_DIR)/wfa.xclbin
endif
endif
